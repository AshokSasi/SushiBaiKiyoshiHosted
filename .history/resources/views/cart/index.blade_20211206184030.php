...
<div class="row g-5">
    <div class="col-md-5 col-lg-4 order-md-last">
      ...
      <ul class="list-group mb-3">
        <li class="list-group-item d-flex justify-content lh-sm">
          <div>
       
            <h6 class="my-0">{{ $menuItems['name'] }} x {{ $menuItems['quantity']}}</h6>
            <small class="text-muted">{{ $menuItems['description'] }}</small>
          </div>
          <p class="text-right">${{ number_format((float)$menuItems['price'],2) }}</p>
            <a href="/contact-list/add" id="addButton" class="btn btn-primary mb-1"><i class="fas fa-plus-circle"></i> </a>
        </li>
     @endforeach
     @endif
