        ...
        <form action="/orders/filter" method="post">
            ...
            <select name="menuFilter" id="menuFilter">
                ...
                @foreach ($menuList as $menuItem)
                    <option value={{ $menuItem->menuItemId }} @if ($menuFilterFlag == $menuItem->menuItemName){{ $var = 'selected' }}@endif>{{ $menuItem->menuItemName }}
                    </option>
                @endforeach
        ...
        <table class="table table-hover table-striped table-dark">
            ...
            <tbody>
                ...
                @if ($statusFilterFlag == 'ALL')
                    @if ($menuFilterFlag == 'ALL')
                        @foreach ($orders as $order)
                            <tr>
                                ...
                                <td> <a href="/orders/show/{{ $order->orderId }}"><button type="submit"
                                            class="btn btn-primary">View</button></a></td>
                            </tr>
                        @endforeach
                        @else
                        @foreach ($orderMenuList as $order)
                        @if ($order->orderMenu->menuItemId == $menuFilterFlag)
                            <tr>
                                <th scope="row">{{ $order->orderId }}</th>
                                <td>{{ $order->userId }}</td>
                                <td>${{ number_format((float) $order->orderTotal, 2) }}</td>
                    ...
                    @endif
                @else
                    {{-- -------------------------ALL status filter menu---------------------------- --}}
                    {{-- @foreach ($orders as $order)
                        @if ($order->orderStatus == $menuFilterFlag)
                            <tr>
                                <th scope="row">{{ $order->orderId }}</th>
                                <td>{{ $order->userId }}</td>
                                <td>${{ number_format((float) $order->orderTotal, 2) }}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form action="/orders/update/{{ $order->orderId }}" method="post">
                                                ...
                                                <select name="status" id="status">
                                                    {{-- <option value="" selected disabled hidden>Choose here</option> --}}
                                                    <option value="In Progress" @if ($order->orderStatus === 'In Progress'){{ $var = 'selected' }} @endif>In Progress
                                                    </option>
                                                    <option value="Completed" @if ($order->orderStatus === 'Completed') {{ $var = 'selected' }}  @endif>Completed</option>
                                                    <option value="Cancelled" @if ($order->orderStatus === 'Cancelled') {{ $var = 'selected' }}  @endif>Cancelled</option>
                                                </select>
