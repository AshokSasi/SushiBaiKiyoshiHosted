...
@section('content')
    ...
    <div class="col-md-8">
        ...
        <form method="POST" action="/menu/edit/{id}" enctype="multipart/form-data">
            {{csrf_field()}}

            <div class="form-group">
                <label for="itemname">Item Name</label>

                <input type="text" class="form-control" id="itemname" name="itemname" value="{{$item->menuItemName}}" required >
            </div>
          
            <div class="form-group">
                <label for="body">Item Description:</label>

                <textarea class="form-control" id="body" rows="3" required>{{$item->menuItemDescription}}</textarea>
            </div>
