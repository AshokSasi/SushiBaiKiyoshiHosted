          ...
          <div class="card shadow-sm">
            ...
            <div class="card-body">
              ...
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                  <a href="/cart/add/{{$menuItem->menuItemId}}"><button type="submit" class="btn btn-sm btn-outline-secondary">Add to Cart</button></a>
                  @if(Auth::check())
                  @if(Auth::user()->userPosition == "a")
                  <a href="/menu/edit/{{ $menuItem->menuItemId}}">
                    <button type="submit" class="btn btn-sm btn-outline-secondary">Edit</button>
                  </a>
                  
                  <!-- Make a form for the delete button action will be the id of the item $menuItem->id  -->
                  <form  method="GET" action="/menu/delete/{{ $menuItem->menuItemId}}">
                   {{csrf_field()}}
                    <button type="submit"  class="btn btn-sm btn-outline-secondary">Delete</button>
               
                  </form>

                  <form  method="POST" action="/menu/discount/{{ $menuItem->menuItemId}}">
                    {{csrf_field()}}
