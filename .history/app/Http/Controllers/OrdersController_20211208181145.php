...
class OrdersController extends Controller
{
    ...
    public function index()
    {
        ...
        //dd($menuItems);
        return view('orders.index', compact('statusFilterFlag','menuFilterFlag'), compact('orders', 'menuList'), compact('menuItems'));
    }

    public function filter(request $request)
    {
        $statusFilterFlag = $request->input('statusFilter');
        $menuFilterFlag = $request->input('itemFilter');
        $orders = Order::with('items')->get();
        $menuItems = OrderedItem::with('orderMenu')->get();
        $menuList = MenuItems::with('menuOrder')->get();

        //dd($menuItems);
        return view('orders.index', compact('orders', 'menuList'), compact('statusFilterFlag'), compact('menuItems'));
    }
